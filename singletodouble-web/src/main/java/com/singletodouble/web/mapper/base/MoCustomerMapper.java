package com.singletodouble.web.mapper.base;

import java.util.List;

import com.singletodouble.web.domain.base.MoCustomer;

/**
 * 【请填写功能名称】Mapper接口
 * 
 * @author ruoyi
 * @date 2026-02-08
 */
public interface MoCustomerMapper {
    /**
     * 查询【请填写功能名称】
     * 
     * @param customerCode 【请填写功能名称】主键
     * @return 【请填写功能名称】
     */
    public MoCustomer searchDataById(String customerCode);

    /**
     * 查询【请填写功能名称】列表
     * 
     * @param moCustomer 【请填写功能名称】
     * @return 【请填写功能名称】集合
     */
    public List<MoCustomer> searchList(MoCustomer moCustomer);

    /**
     * 新增【请填写功能名称】
     * 
     * @param moCustomer 【请填写功能名称】
     * @return 结果
     */
    public int insertData(MoCustomer moCustomer);
    
    /**
     * 批量新增【请填写功能名称】
     * 
     * @param moCustomer 【请填写功能名称】
     * @return 结果
     */
    public int insertDataList(List<MoCustomer> dataList);

    /**
     * 修改【请填写功能名称】
     * 
     * @param moCustomer 【请填写功能名称】
     * @return 结果
     */
    public int updateData(MoCustomer moCustomer);

    /**
     * 删除【请填写功能名称】
     * 
     * @param customerCode 【请填写功能名称】主键
     * @return 结果
     */
    public int deleteDataById(String customerCode);

    /**
     * 批量删除【请填写功能名称】
     * 
     * @param customerCodes 需要删除的数据主键集合
     * @return 结果
     */
    public int deleteDataByIds(String[] customerCodes);
}
